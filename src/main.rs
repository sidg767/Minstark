...
println!("inputs    : {:?}", inputs.iter().map(|x| x.as_int()).collect::<Vec<_>>());
...
let ok = verifier.verify(&proof, proof.root, trace.length, &trace.values);
println!("\nverifier result: {}", if ok { "ACCEPTED" } else { "REJECTED" });
}
